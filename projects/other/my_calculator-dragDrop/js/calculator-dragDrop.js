...
function calculate(operation) {
    ...
    saveToHistory(inputField.value, operation, resultText);
}

function saveToHistory(input, operation, result) {
    const history = document.getElementById("history");
    const historyItem = document.createElement("div");
    historyItem.className = "history-item";
    historyItem.innerHTML = `<strong>${operation}</strong> on <em>${input}</em> = <strong>${result}</strong>`;
